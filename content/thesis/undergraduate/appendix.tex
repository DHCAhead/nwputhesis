...
\begin{itemize}
    \item 代码主目录：\texttt{04-code/nurbs-code/}（核心文件包括 \texttt{model.py}、\texttt{trainer.py}、\texttt{train\_ar.py}、\texttt{generate.py}、\texttt{sequence2nurbs.py}、\texttt{utils.py}、\texttt{eval\_generated\_brep.py}、\texttt{dataset.py} 等）。
    \item 数据预处理脚本：\texttt{04-code/nurbs-code/2brep\_nurbs\_wcs.py}、\texttt{2brep\_nurbs\_quan\_wcs.py}；服务器侧镜像位于 \texttt{experiment-local/2sequence\_nurbs.py} 与 \texttt{experiment-local/2brep.py}。
    \item 训练数据：\texttt{data/nurbs\_sequences\_v1\_patch.pkl}，DeepCAD 子集，训练 64\,325 / 验证 6\,095，数据词表 4\,053。
    \item 实验配置与日志：\texttt{03-experiments/configs/}、\texttt{03-experiments/runs/}、\texttt{03-experiments/metrics/}、\texttt{03-experiments/toloacl/}（含 \texttt{hr2\_patch1\_\_Val\_CE\_Loss.csv} 等关键曲线数据）。
    \item 训练日志：\texttt{experiment-local/logs/hr2\_patch1.log}（含模型初始化打印、每 5 epoch 验证最优 checkpoint 记录）。
\end{itemize}
